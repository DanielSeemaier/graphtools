...
public:
  MultiColumnProgressBar(const std::array<std::size_t, kNumColumns> &widths,
                         const std::array<std::size_t, kNumColumns> &max_values,
  ...
  void finish(const std::size_t column) { _values[column] = _max_values[column]; }

  void finish() { _final_render = true; }
  ...
  void render(std::ostream &out) {
    ...
    if (_final_render) { std::cout << std::endl; }
  }

private:
  void render_captions(std::ostream &out) {
    if (has_captions()) {
      if (!_prefix.empty()) { out << std::string(_prefix_width + 1, ' '); }
  ...
  void render_progress_bars(std::ostream &out) {
    if (!_prefix.empty()) {
      if (_prefix_width != 0) { out << std::setfill(' ') << std::setw(_prefix_width); }
  ...
  [[nodiscard]] char spinner_sym(const std::size_t column) const { return kSpinner[_spinner_index[column]]; }
